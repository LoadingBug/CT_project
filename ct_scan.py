...
def ct_scan(photons, material, phantom, scale, angles, order=1, mas=10000):
	...
	for angle in range(angles):
		...
		thickness = np.zeros((len(material.coeffs), n))

		for index, m in enumerate(materials):
			interpolated = scipy.ndimage.map_coordinates(material_phantom[index], [y0, x0], order=order, mode='constant', cval=0, prefilter=False)
			thickness[m] = np.sum(interpolated, axis=0)
		...
		thickness[air] = 2 * n - np.sum(thickness, axis=0)

		# scale the thickness appropriately and calculate detections for this set of
		# materials
		thickness *= scale
	
		scan[angle] = ct_detect(photons, material.coeffs, thickness, mas)
